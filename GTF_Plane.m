...
nose = translate(scale(tmpsphere, 3, 1, 1), -4.5 + position(1), 0, 1.01 + position(3));
tailcap = translate(scale(tmpsphere, 2, 1, 1), 4.5 + position(1), 0, 1.01 + position(3));
cyl1 = UnitCylinder(4);
cyl1 = translate(rotateY(scale(cyl1, 1, 1, 5), 90), position(1), 0, 1.01 + position(3));
cyl1.facecolor = fuselage_colour;
fuselage = combine(cyl1, nose, tailcap);

% Fins and rudders
fins = translate(scale(UnitSphere(2), .75, 3, .2), 5 + position(1), 0, 1 + position(3));
fins.facecolor = 'blue';
wings = translate(scale(UnitSphere(2), 1.5, 8, .2), -1 + position(1), 0, 1 + position(3));
wings.facecolor = 'blue';
rudders = translate(rotateX(scale(UnitSphere(2), .75, 2, .2), 90), 5 + position(1), 0, 0 + position(3));
rudders.facecolor = 'blue';
foils = combine(fins, rudders, wings);

% Motors and propellers
motor = scale(UnitSphere(2), 1.5, .5, .5);
motor.facecolor = 'black';
motors = translate(motor, 5.5 + position(1), 0, 1 + position(3));
prop = rotateX(scale(UnitSphere(1.5), .1, 2, .2), time * 90);
prop.facecolor = 'black';
prop = translate(prop, 6.5 + position(1), 0, 1 + position(3));
