...
function [Tracker, Sims] = EventTriggeredDEKF(Tracker,Ts,t,Sims,Gains)
...
% Tuning parameters
    ...
    if rem(t,T_meas)==0 
        ...
        if max(alpha_values) > Gains.pi_max 
            %   event triggered, fussion
            Sims.event(end+1) = 1;
            for i = 1:Sims.N
            ...
            %   event not triggered, propagation
            Sims.event(end+1) = 0;
            for i = 1:Sims.N
                for j = 1:Sims.M
                    Tracker(i).Omega_cor(:,:,j) = Tracker(i).Omega_tilde(:,:,j);                     
                    Tracker(i).Omega_cor(:,:,j)\Tracker(i).z_tilde(:,j);
                    Tracker(i).q_hat_cor(:,j) = Tracker(i).Omega_cor(:,:,j)\Tracker(i).z_tilde(:,j);
                end
            end
        end
    else
